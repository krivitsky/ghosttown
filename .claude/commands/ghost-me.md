...
6. **Persist the choice.** Once you've resolved the ghost slug — whether passed directly or picked from the list in a follow-up — run `node hooks/ghost-set.js <slug>` to write the persistence flag. (The single-shot `/ghost-me <slug>` form is also caught by a hook, but the picker path relies on this step.)
