...
Write `ghosts/[slug]-eval.md` — summary table at the very top (axes above, winner per axis), — split the table with two label rows: a `| **Summary** | | | |` row above the **Voice-tic hit rate** row (which is **bold** with the gain in its winner cell, `**B +NNN%**`, as the headline metric), and a `| **Details** | | | |` row above all the remaining axes — then a **headline line** stating it: `**Voice-fidelity gain: +NNN%** (naked_hits/total → ghost_hits/total, naked → ghost)`, the same `+NNN%` that goes in the README Eval column — then one bottom-line sentence, then: the probe, both responses verbatim, the voice-feature scorecard, and the assessment. See `ghosts/donald-trump-eval.md` as the reference format.
